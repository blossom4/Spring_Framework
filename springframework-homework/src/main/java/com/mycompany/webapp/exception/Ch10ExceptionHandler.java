...
public class Ch10ExceptionHandler {
	...
	public String handleNullPointerException(NullPointerException e) {
		logger.info("Run error/500_null");
		e.printStackTrace();
	...
	public String handleClassCastException(ClassCastException e) { // 타입 변환이 잘못될 시에 발생하는 예외
		logger.info("Run error/500_cast");
		e.printStackTrace();
	 ...
	 * handleCh16NotFoundAccountException(Ch16NotFoundAccountException e, Model
	 * model) { // 타입 변환이 잘못될 시에 발생하는 예외 logger.info("실행"); e.printStackTrace();
	 * model.addAttribute("error", e.getMessage()); return
	 * "error/notFoundAccountException"; }
	 */
